    ...
    export default {
        ...
        methods: {
            ...
            */
            findComment(){
                let _this = this;
                _this.$ajax.get(process.env.VUE_APP_SERVER + '/business/web/comment/find/' + _this.courseid).then((response)=>{
            ...
            sendComment(){
                ...
                if(Tool.isEmpty(this.replyComment)){
                    ...
                    // 传参录入数据库
                    let date = new Date()
                    _this.comment.name = _this.myName
                    _this.comment.headimg = _this.myHeader
                    _this.comment.comment = _this.replyComment
                    _this.comment.commentnum = 0
                    _this.comment.time = _this.dateFormat("YYYY-mm-dd HH:MM", date)
                    _this.comment.like = 0
                    _this.comment.courseid = _this.$route.query.id

                    _this.$ajax.post(process.env.VUE_APP_SERVER + '/business/web/comment/save', _this.comment).then((response)=>{
                        let resp = response.data;
                        if (resp.success) {
                            _this.findComment();
                            Toast.success("保存成功！");
